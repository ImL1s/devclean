...
| Antigravity | [MCP 伺服器按工作區產生，從不清理](https://discuss.ai.google.dev/t/bug-mcp-servers-spawn-per-workspace-causes-process-explosion-and-10-gb-idle-ram/129054) | 進程爆炸；閒置時佔用約 10 GB 記憶體 |
...
| FVM | FVM 管理的 `dart`/`flutter` 進程可能卡住（例如 `fvm use` 等待互動輸入） | 卡住的進程累積，每個佔 100% CPU |
...
#### 6. Logi Options+：已知的記憶體洩漏

`logioptionsplus_agent` 有一個[已被廣泛記錄的記憶體洩漏問題](https://logitech.uservoice.com/forums/925117-logi-options/suggestions/45330115-memory-leak-with-the-latest-logi-options)，已被回報超過 3 年。該代理啟動時約佔 100 MB，但[隨時間增長至數 GB](https://canartuc.medium.com/if-you-are-short-on-ram-nowadays-check-logi-options-2bfcf1187500)——極端案例高達 36 GB。由於 `launchd` 會自動重啟它，唯一實際的解決方案是定期殺掉進程讓它重新啟動。Logitech 至今尚未完全修復此問題。
...
| **深度模式分離** | 重型 daemon（Gradle、Kotlin LSP、FVM、Antigravity Language Server、Logi Options+）需要明確的 `--deep` 旗標；xcodebuild 更進一步限制只殺孤兒 | 安全 — 需主動選擇，不會意外觸發 |
...
> **警告：** 深度模式會殺掉非孤兒的 Gradle、Kotlin LSP、Flutter daemon、FVM 進程、Antigravity Language Server、Logi Options+ 代理及 Ruby/Fastlane 進程。如果正在進行建置或編譯，可能會被中斷。請先用 `--dry-run` 預覽會被影響的進程。
...
### 優化模式（`--optimize`）
套用一次性修復以減少資源浪費：停用 VS Code / Cursor / Antigravity / Kiro 的 crash reporter，清理 Crashpad dump 檔案，並停用已知的資源浪費背景代理。
...
在 `devclean` 腳本中編輯 `ORPHAN_PATTERNS` 陣列即可加入你的工具：
